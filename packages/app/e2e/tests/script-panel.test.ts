...
async function setupScriptMocks(electronApp: ElectronApplication): Promise<void> {
...
test.describe.serial('Script Panel', () => {
  test('Setup mocks and reset store', async ({ page, electronApp }) => {
    await setupScriptMocks(electronApp)
  ...
  test('File selection via dialog mock', async ({ page, electronApp }) => {
    await electronApp.evaluate(({ ipcMain }) => {
      ipcMain.removeHandler('dialog:open-file')
      ipcMain.handle('dialog:open-file', () => '/tmp/test-script.yaml')
    })
